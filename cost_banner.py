...
CSS = """
.costban{display:block;border:1px solid var(--rule);border-left:5px solid var(--shu);background:var(--surface);
  margin-top:26px;padding:15px 18px;text-decoration:none;color:var(--ink)}
.costban:hover{background:var(--sunk)}
.costban .cbk{font-family:var(--cond);font-weight:600;font-size:11.5px;letter-spacing:.05em;color:var(--shu)}
.costban .cbmain{display:flex;flex-wrap:wrap;align-items:baseline;gap:10px;margin-top:6px}
.costban .cbu{font-family:var(--mono);font-size:19px;font-variant-numeric:tabular-nums}
.costban .cbeq{color:var(--ink3);font-size:13px}
.costban .cbv{font-family:var(--mono);font-size:23px;font-variant-numeric:tabular-nums;color:var(--ink);
  transition:opacity .18s}
.costban .cbv.fade{opacity:.25}
.costban .cbgo{margin-left:auto;font-family:var(--cond);font-size:12.5px;color:var(--shu)}
.costban input[type=range]{width:100%;margin-top:10px;accent-color:var(--shu)}
.costban .cbn{display:block;margin-top:8px;font-size:11.5px;line-height:1.8;color:var(--ink3)}
...
def banner(basis: dict) -> str:
  ...
  <a class="costban" href="{SITE_URL}cost/" id="costban">
    ...
    <span class="cbmain">
      <span class="cbu"><b id="cbu">{UNIT_DEFAULT}</b> 戸</span>
