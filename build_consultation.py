...
# 集合住宅のときだけ聞く。**所有形態で相談相手も段取りも変わる。**
#   分譲 … 決議の時期が動かせない。管理会社・コンサルが間に入ることが多い
#   賃貸 … オーナーの決裁。入居や稼働への影響が主な論点
TENURES = [
    ("分譲マンション", "管理組合・管理会社・修繕委員会が窓口"),
    ("賃貸・社宅（ワンオーナー）", "オーナー様・資産管理会社が窓口"),
    ("わからない・その他", "これから確認する"),
]
...
def _choices(items: list[tuple[str, str]], kind: str, group: str) -> str:
    ...
    for value, note in items:
        out.append(
            ...
            f'value="{value}" /><b>{value}</b><span>{note}</span></label>')
    return "".join(out)
