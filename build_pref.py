...
.finder input:focus{outline:2px solid var(--ai);outline-offset:1px;border-color:var(--ai)}
.qnote{margin:8px 0 0;min-height:1.2em;font-family:var(--mono);font-size:12px;color:var(--ink2)}
...
# 新しい文言は **元請業者**と**管理会社**、つまり事業者だけに向けている。
# 管理会社は元請候補そのものなので（既存元請のフジタBM・リンクス・ビルドも
# 管理・ビルメン系）、元請を飛び越す動きにはならず、むしろ元請プールが広がる。
#
# **建物オーナーに直接呼びかけないのは本人判断**（2026-09-12）。理由は3つ。
# (1) 元請・管理会社を飛び越す形になる、(2) 発注に不慣れで手間がかかる、
# (3) 見積だけ取って終わる比率が高い。**取れない引き合いは市場ではない。**
# 末尾の「等へ」は、自分から名乗り出るオーナーの入口を閉じずに濁すための語。
# 建物の種類は .d（12.5px）に移してある。.k は 10.5px・字間広めで、
# 長い文言を入れるとスマホで細かい灰色文字が3行になる。
#
# **「大規模修繕をご検討中の方へ」に戻さないこと。**分譲マンションの管理組合＝
...
def build(basis: dict) -> int:
    ...
    なお<strong>この数字は分譲と賃貸を合わせたものです</strong>。
    この統計表で絞り込めるのは「非木造の共同住宅」までなので、賃貸マンションを含みます。
    持ち家かどうかの内訳は、同じ調査の別の統計表から各市区のページに載せています。
  </div>
...
''')
  ...
  <div class="warn">
    <h4>この数字が指しているもの</h4>
    <ul>
      <li><strong>見出しの戸数は、分譲と賃貸を合わせた数です。</strong>この統計表で絞り込めるのは「非木造の共同住宅」までで、賃貸マンションを含みます。持ち家かどうかの内訳は、各市区のページに別の統計表から載せています。</li>
